...
pub trait Handler: Send + Sync + 'static {
    async fn invoke(&self, context: Context, post_db: Arc<RwLock<Vec<Post>>>) -> Response;
}

#[async_trait]
impl<F: Send + Sync + 'static, Fut> Handler for F
where
    F: Fn(Context, Arc<RwLock<Vec<Post>>>) -> Fut,
    Fut: Future + Send + 'static,
    Fut::Output: IntoResponse,
{
    async fn invoke(&self, context: Context, post_db: Arc<RwLock<Vec<Post>>>) -> Response {
        (self)(context, post_db).await.into_response()
    }
}
...
async fn not_found_handler(_cx: Context, _: Arc<RwLock<Vec<Post>>>) -> Response {
