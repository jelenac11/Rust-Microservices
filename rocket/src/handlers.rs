...
pub fn get_post(id: i32, connection: DbConn) -> Result<Json<Post>, Status> {
    repository::get_post(id, &connection)
        .map(|post| Json(post))
        .map_err(|error| error_status(error))
}
...
pub fn update_post(id: i32, post: Json<Post>, connection: DbConn) -> Result<Json<Post>, Status> {
    repository::update_post(id, post.into_inner(), &connection)
        .map(|post| Json(post))
        .map_err(|error| error_status(error))
}

#[post("/<id>")]
pub fn publish_post(id: i32, connection: DbConn) -> Result<Json<Post>, Status> {
    repository::publish_post(id, &connection)
        .map(|post| Json(post))
        .map_err(|error| error_status(error))
}
...
pub fn delete_post(id: i32, connection: DbConn) -> Result<status::NoContent, Status> {
    repository::delete_post(id, &connection)
